...
double dp[3001][3001];
int limit;
double helper(int n,int head,vector<double> &v){
    if(n < 0){
        return 0.0;
    }
    if(n == 0 && head > limit){
    ...
    // take head 
    double op1 = helper(n-1,head + 1,v) * v[n];
    double op2 = helper(n-1,head,v) * (1 - v[n]);
    return dp[n][head] = op1 + op2;
}
