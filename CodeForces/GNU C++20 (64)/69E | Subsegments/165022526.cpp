...
void solve() {
    ...
    for(int i = k + 1;i <= n;i ++){
        // we are removing (i - k)th index
        int toRemove = v[i-k];
        mp1[toRemove] -= 1;
        if(one.find(toRemove) != one.end()){
            one.erase(one.find(toRemove));
        }
        if(mp1[toRemove] == 1){
            one.insert(toRemove);
        }
 
        // we are adding the ith element
        int toAdd = v[i];
        mp1[toAdd] += 1;
        if(one.find(toAdd) != one.end()){
            one.erase(one.find(toAdd));
        }
        if(mp1[toAdd] == 1){
            one.insert(toAdd);
        }
